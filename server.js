...
try {
    ...
    return { audioUrl: result.secure_url, duration: duration };
} catch (error) {
...
try {
    ...
    res.status(200).json({ audioUrl: result.audioUrl, duration: result.duration });
} catch (error) {
...
app.post('/create-video-with-subtitles', async (req, res) => {
    ...
    try {
      const result = await runVideoCreation(imageSources, audioUrl, audioDuration, topic, height, width);
